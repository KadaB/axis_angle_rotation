...
\items{
\item Setup up explizite plane equation placed orthogonal rotation axis(normal) and rotate vector along plane. \\  
\item Points (x,y) on plain P (${R^3}$): \\ $P(x, y) =  \vec{c} + x \, \vec{u} +  y \, \vec{v}$ with $\vec{u} \bullet \vec{v} = 0$ \\ 
unit circle on plane: $P(\cos(\theta), \sin(\theta)) =  \vec{c} + \cos(\theta) \, \vec{u} +  \sin(\theta) \, \vec{v}$ 
}
