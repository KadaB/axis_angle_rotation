...
$\gleichung{
    \vec{v'}
      ...
      & =  (\vec{v} \bullet \hat{n} ) \hat{n} + \cos(\theta) \left(\vec{v} - ( \vec{v} \bullet \hat{n} ) \hat{n} \right) + \sin(\theta) \, \hat{n} \times \vec{v} &|& (\vec{n} \bullet \hat{n}) \hat{n} = (\hat{n}^T \hat{v})\hat{n} = \hat{n}\hat{n}^T \vec{v}\\
...
$\mathbf{R}(\hat{n}, \theta) =  \mathbf{P} + \cos(\theta) \left( \mymat{ 1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1 } - \mymat { n_x^2 & n_x n_y & n_x n_z  \\ n_x n_y & n_y^2 & n_y n_z  \\ n_x n_z & n_y n_z & n_z^2  \\ } \right) + \sin(\theta) \, \mymat{ 0 & -n_z & n_y \\ n_z & 0 & -n_x \\ -n_y & n_x & 0 } $
}
